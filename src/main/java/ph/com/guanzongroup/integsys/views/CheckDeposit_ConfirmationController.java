    ...
    private void handleButtonAction(ActionEvent event) {
        try {
            ...
            switch (lsButton) {
                ...
                case "btnVoid":
                    if (ShowMessageFX.YesNo(null, psFormName, "Do you want to void this transaction?")) {
...
//            makeClearableReadOnly(tfFieldName);
    ...
    private void initTableMaster() {
        ...
        tblColBankAccountNo.setCellValueFactory(new PropertyValueFactory<>("index04"));
        tblColBankAccountNme.setCellValueFactory(new PropertyValueFactory<>("index05"));
        ...
        Task<Void> task = new Task<Void>() {
            ...
            protected Void call() throws Exception {
                try {
                    ...
                    if ("success".equals(poJSON.get("result"))) {
                        if (poGLControllers.CheckDeposits().getCheckTransferMasterCount()> 0) {
                            for (int lnCntr = 0; lnCntr < poGLControllers.CheckDeposits().getCheckTransferMasterCount(); lnCntr++) {
                                main_data.add(new ModelTableMain(
                                        String.valueOf(lnCntr + 1),
                                        poGLControllers.CheckDeposits().poCheckDepositMaster(lnCntr).getTransactionNo() == null ? ""
                                        : poGLControllers.CheckDeposits().poCheckDepositMaster(lnCntr).getTransactionNo(),
                        ...
                        switch (txtFieldID) {
                            case "tfBankMaster":
                                ...
                                if ("error".equals(poJSON.get("result"))) {
                                 ...
                                 tfBankMaster.setText(poGLControllers.CheckDeposits().Master().BankAccount().Banks().getBankName());
                                break;
                            case "tfBankAccountNo":
                                poJSON = poGLControllers.CheckDeposits().SearchBankAccounts(lsValue,false);
                                if ("error".equals(poJSON.get("result"))) {
