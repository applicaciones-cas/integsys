...
//                            poController.POQuotationRequest().initFields();
                    ...
                    case "btnHistory":
                        ...
                        try {
                            poController.POQuotationRequest().ShowStatusHistory();
                        ...
                        try {
                            ...
                            for (lnCtr = 0; lnCtr < poController.POQuotationRequest().getDetailCount(); lnCtr++) {
                                if (poController.POQuotationRequest().Detail(lnCtr).isReverse()) {
                                    ...
                                    lnRowCount += 1;
                                    double lnTotal = poController.POQuotationRequest().Detail(lnCtr).getQuantity() * poController.POQuotationRequest().Detail(lnCtr).getUnitPrice(); //Inventory().getCost()
                                    details_data.add(
