...
//                    pbIsFromBrowse = false;
                ...
                case "btnNew":
                    ...
                    tfVoucherNo.setText(poController.getVoucherNo());
                    poController.Master().setVoucherNo(tfVoucherNo.getText());
    ...
    public void loadHighlightFromDetail() {
        try {
            for (int lnCtr = 0; lnCtr < poController.getDetailCount(); lnCtr++) {
                ...
                String lsTransType = !JFXUtil.isObjectEqualTo(poController.Detail(lnCtr).Particular().getAccountCode(), null, "") ? poController.Detail(lnCtr).Particular().getAccountCode() : "";
                                    ...
                                    if (unifiedPayments != null && !unifiedPayments.isEmpty()) {
                                        for (Object requestObj : unifiedPayments) {
                                            ...
                                            ModelDisbursementVoucher_Main loMain = new ModelDisbursementVoucher_Main(
                                                    ...
                                                    (obj.get("sTransNox") != null ? obj.get("sTransNox").toString() : "" + obj.get("TransactionType") != null ? obj.get("TransactionType").toString() : "")
                                            );
...
//                            }
                        ...
                        try {
                            if (pnEditMode == EditMode.UPDATE) {
                                if ((poController.Journal().getDetailCount() - 1) >= 0) {
                                    if (poController.Journal().Detail(poController.Journal().getDetailCount() - 1).getAccountCode() != null
                                        ...
                                        poController.Journal().Detail(poController.Journal().getDetailCount() - 1).setForMonthOf(oApp.getServerDate()).equals("");
                                    }
                                }
                            }
                            ...
                            if (pnDetailJE <= 0) {
                                if (!journal_data.isEmpty()) {
                                    ...
                                    loadRecordDetailJE();
                                    if (poController.Journal().Detail(pnDetailJE).getAccountCode() == null) {
    ...
    private void initTableOnClick() {
        ...
        JFXUtil.applyRowHighlighting(tblViewMainList, item -> ((ModelDisbursementVoucher_Main) item).getIndex08(), highlightedRowsMain);
                        ...
                        if (lsValue.isEmpty()) {
                            if (!tfPayeeName.getText().isEmpty()) {
                                ...
                                if (poController.getDetailCount() >= 1) {
                                    if (poController.Detail(0).getSourceNo() != null) {
                                        if (!poController.Detail(0).getSourceNo().isEmpty()) {
                                            if (!isExchangingSupplier()) {
                                                return;
                                            }
                                        }
...
//                        }
//                        if (lsValue == null || lsValue.isEmpty()) {
//                            poController.Detail(pnDetail).setTaxCode(null);
//                            poController.Detail(pnDetail).setTaxAmount(DisbursementStatic.DefaultValues.default_value_double_0000);
//                            poController.Detail(pnDetail).setTaxRates(DisbursementStatic.DefaultValues.default_value_double_0000);
//                            poController.Detail(pnDetail).setDetailVatAmount(DisbursementStatic.DefaultValues.default_value_double_0000);
//                            poController.Detail(pnDetail).isWithVat(false);
...
//                                    poController.Detail(pnDetail).getDetailVatAmount(),
...
//                                loadTableMain.reload();
                            ...
                            case "tfSupplier":
                                ...
                                poJSON = poController.SearchSupplier(lsValue.replace(" ", ""), false);
                                if ("error".equals((String) poJSON.get("result"))) {
                                    ShowMessageFX.Warning((String) poJSON.get("message"), pxeModuleName, null);
...
//                                loadTableMain.reload();
                            ...
                            case "tfBankNameCheck":
                                ...
                                tfBankNameCheck.setText(poController.Master().getDisbursementType().equals(
                                        DisbursementStatic.DisbursementType.CHECK)
                                                ? (poController.CheckPayments().getModel().Banks().getBankName() != null
                                                ? poController.CheckPayments().getModel().Banks().getBankName() : "") : "");
                                break;
                            case "tfBankAccountCheck":
                                ...
                                tfBankNameCheck.setText(poController.CheckPayments().getModel().Banks().getBankName());
                            ...
                            case "tfPayeeName":
                                ...
                                psSupplierPayeeId = poController.Master().getSupplierClientID();
                                tfSupplier.setText(poController.Master().Payee().Client().getCompanyName() != null ? poController.Master().Payee().Client().getCompanyName() : "");
                                tfPayeeName.setText(poController.Master().getDisbursementType().equals(DisbursementStatic.DisbursementType.CHECK) ? poController.Master().Payee().getPayeeName() : "");
                                loadRecordMasterBankTransfer();
                                break;
                            case "tfAuthorizedPerson":
    ...
    public void moveNext(boolean isUp, boolean continueNext) {
        try {
            ...
            JFXUtil.requestFocusNullField(new Object[][]{ // alternative to if , else if
                {poController.Detail(pnDetail).Particular().getDescription(), tfParticularsDetail},
    ...
    private void loadRecordMaster() {
        try {
            ...
            cmbPaymentMode.getSelectionModel().select(!poController.Master().getDisbursementType().equals("") ? Integer.valueOf(poController.Master().getDisbursementType()) : -1);

            if (poController.Master().getEditMode() == EditMode.UPDATE) {
    ...
    private void loadRecordMasterCheck() {
        try {
            ...
            cmbPayeeType.getSelectionModel().select(!poController.CheckPayments().getModel().getPayeeType().equals("") ? Integer.valueOf(poController.CheckPayments().getModel().getPayeeType()) : -1);
            cmbDisbursementMode.getSelectionModel().select(!poController.CheckPayments().getModel().getDesbursementMode().equals("") ? Integer.valueOf(poController.CheckPayments().getModel().getDesbursementMode()) : -1);
            cmbClaimantType.getSelectionModel().select(!poController.CheckPayments().getModel().getClaimant().equals("") ? Integer.valueOf(poController.CheckPayments().getModel().getClaimant()) : -1);
            ...
            cmbCheckStatus.getSelectionModel().select(!poController.CheckPayments().getModel().getTransactionStatus().equals("") ? Integer.valueOf(poController.CheckPayments().getModel().getTransactionStatus()) : -1);

            if (!chbkPrintByBank.isSelected()) {
                JFXUtil.setDisabled(true, cmbPayeeType, cmbDisbursementMode, cmbClaimantType, tfAuthorizedPerson);
                clearFilter();
            }
        } catch (SQLException | GuanzonException ex) {
            Logger.getLogger(getClass()
    ...
    private void loadRecordMasterBankTransfer() {
        ...
        cmbOtherPaymentBTransfer.getSelectionModel().select(0);
            ...
            (cmbId, selectedIndex, selectedValue) -> {
                switch (cmbId) {
                    ...
                    case "cmbDisbursementMode":
                        if (poController.CheckPayments().getModel().getDesbursementMode().equals("1")
...
//                                    cmbDisbursementMode.getSelectionModel().select(!poController.CheckPayments().getModel().getDesbursementMode().equals("") ? Integer.valueOf(poController.CheckPayments().getModel().getDesbursementMode()) : -1);
                                    cmbClaimantType.getSelectionModel().select(!poController.CheckPayments().getModel().getClaimant().equals("") ? Integer.valueOf(poController.CheckPayments().getModel().getClaimant()) : -1);
//                                    tfAuthorizedPerson.setText(poController.CheckPayments().getModel().getAuthorize());
                                    loadRecordMasterCheck();
                                });
                                return;
                            }
                        } else if (poController.CheckPayments().getModel().getDesbursementMode().equals("1")
                        && poController.CheckPayments().getModel().getClaimant().equals("0")) {
                    ...
                    case "cmbClaimantType":
                        if (poController.CheckPayments().getModel().getClaimant().equals("0") && !tfAuthorizedPerson.getText().isEmpty()) {
                        ...
                        poController.CheckPayments().getModel().setClaimant(String.valueOf(cmbClaimantType.getSelectionModel().getSelectedIndex()));
                        initFields(pnEditMode);
                        break;
            ...
            if (source instanceof DatePicker) {
                ...
                switch (datePicker.getId()) {
                    case "dpCheckDate":
                        ...
                        if (poController.CheckPayments().getEditMode() == EditMode.ADDNEW
                                || poController.CheckPayments().getEditMode() == EditMode.UPDATE) {
        ...
        if (source instanceof CheckBox) {
            ...
            switch (checkedBox.getId()) {
                case "chbkPrintByBank": // this is the id
                    if ((pnEditMode == EditMode.ADDNEW || pnEditMode == EditMode.UPDATE)) {
                        poController.Master().setBankPrint(chbkPrintByBank.isSelected() == true ? "1" : "0");
                        if (!chbkPrintByBank.isSelected()) {
                            cmbPayeeType.setDisable(true);
                            cmbDisbursementMode.setDisable(true);
                            cmbDisbursementMode.setDisable(true);
    ...
    private boolean isExchangingPayeeName() {
        ...
        try {
            ...
            if (ShowMessageFX.YesNo("DV Details have already items, are you sure you want to change payee name?", pxeModuleName, null)) {
                ...
                for (int i = poController.getDetailCount() - 1; i >= 0; i--) {
                    if (poController.Detail(i).getSourceNo().isEmpty()
                            && poController.Detail(i).getAmount() == 0.0000) {
                        continue;
                    }
                    poController.Detail().remove(i);
                }
                ...
                if (poController.Journal().Master().getTransactionNo() != null) {
                    for (int i = poController.Journal().getDetailCount() - 1; i >= 0; i--) {
                        if (poController.Journal().Detail(i).getAccountCode() == null) {
                            continue;
                        }
                        poController.Journal().Detail().remove(i);
                    }
                    pnDetailJE = -1;
                    JFXUtil.clearTextFields(apJournalDetails);
                    loadTableDetailJE.reload();
