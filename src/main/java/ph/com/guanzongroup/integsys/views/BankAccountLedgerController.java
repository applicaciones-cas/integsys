...
import ph.com.guanzongroup.integsys.model.ModelCashFundLedger;
...
public class BankAccountLedgerController implements Initializable {
    ...
    int pnEntryNo = 0;
    int pnDetail = 0;
    private final String pxeModuleName = "Cash Fund Ledger";
    static BankAccountMaster oCashflow;
    private String psSearchDateFrom = "";
    private String psSearchDateTo = "";
    private ObservableList<ModelCashFundLedger> details_data = FXCollections.observableArrayList();
    private FilteredList<ModelCashFundLedger> filteredMain_Data;
    ...
    @FXML
    private TableView<ModelCashFundLedger> tblViewDetail;
    @FXML
    private TableColumn tblRowNoDetail, tblBankAcctID, tblLedgerNo, tblTransDate, tblSourceCode, tblSourceNo, tblDebit, tblCredit;
    @FXML
    private Pagination pagination;

    public void setObject(BankAccountMaster foObject) {
        oCashflow = foObject;
    }
        ...
        loadTableDetail = new JFXUtil.ReloadableTableTask(
                ...
                () -> {
                    Platform.runLater(() -> {
                        ...
                        for (int lnCtr = 0; lnCtr < oCashflow.getLedgerListCount(); lnCtr++) {
                            if (!JFXUtil.isObjectEqualTo(oCashflow.LedgerList(lnCtr).getTransactionDate(), null, "")) {
                                date = sdf.format(oCashflow.LedgerList(lnCtr).getTransactionDate());
                            }
                            lnRowCount += 1;
                            details_data.add(
                                    new ModelCashFundLedger(String.valueOf(lnRowCount),
                                            date,
                                            String.valueOf(oCashflow.LedgerList(lnCtr).getLedgerNo()),
                                            String.valueOf(oCashflow.LedgerList(lnCtr).getSourceCode()),
                                            String.valueOf(oCashflow.LedgerList(lnCtr).getSourceNo()),
                                            CustomCommonUtil.setIntegerValueToDecimalFormat(oCashflow.LedgerList(lnCtr).getAmountIn(), true),
                                            CustomCommonUtil.setIntegerValueToDecimalFormat(oCashflow.LedgerList(lnCtr).getAmountOut(), true),
                                            String.valueOf(oCashflow.LedgerList(lnCtr).getBankAccountId()),
    ...
    private void retrieveLedger() {
        try {
            ...
            poJSON = oCashflow.loadLedgerList(psSearchDateFrom, psSearchDateTo);
...
//                ShowMessageFX.Error(null, pxeModuleName, JFXUtil.getJSONMessage(poJSON));
    ...
    private void initDetailGrid() {
        JFXUtil.setColumnCenter(tblRowNoDetail, tblBankAcctID,tblLedgerNo, tblTransDate, tblSourceCode, tblSourceNo, tblDebit, tblCredit);
        JFXUtil.setColumnLeft(tblSourceNo);
