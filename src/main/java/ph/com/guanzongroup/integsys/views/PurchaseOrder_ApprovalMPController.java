 ...
 * @author johndave
 */
public class PurchaseOrder_ApprovalMPController implements Initializable, ScreenInterface {
    ...
    private void loadRecordDetail() {
        try {
            ...
            if (pnTblDetailRow >= 0) {
                ...
                tfOrderQuantity.setText(CustomCommonUtil.setIntegerValueToDecimalFormat(poPurchasingController.PurchaseOrder().Detail(pnTblDetailRow).getQuantity().intValue()));
            }
        } catch (GuanzonException | SQLException ex) {
            Logger.getLogger(PurchaseOrder_ApprovalMPController.class.getName()).log(Level.SEVERE, null, ex);
        }
...
//                    }
    ...
    private void initTextFieldFocus() {
        List<TextField> loTxtField = Arrays.asList(tfReferenceNo, tfDiscountRate, tfDiscountAmount,
                tfAdvancePRate, tfAdvancePAmount, tfOrderQuantity, tfCost);
    ...
    private void txtField_KeyPressed(KeyEvent event) {
        ...
        try {
            ...
            Logger.getLogger(PurchaseOrder_ApprovalMPController.class
    ...
    private void setOrderCost(String fsValue) {
        ...
        double lnCostDetail = Double.parseDouble(tfCost.getText().replace(",", ""));
        if (lnCostDetail < 0.00) {
            ShowMessageFX.Warning("Invalid Order Cost", psFormName, null);
            fsValue = "0.00";
        }

        tfCost.setText(fsValue);
            ...
            if (pnEditMode == EditMode.ADDNEW || pnEditMode == EditMode.UPDATE) {
                ...
                if (tfTotalAmount.getText().isEmpty()
                    ...
                    if (chkbAdvancePayment.isSelected()) {
                        ...
                        if (Double.parseDouble(tfAdvancePAmount.getText().replace(",", "")) > 0.00
    ...
    private void initFields(int fnEditMode) {
        ...
        if (poPurchasingController.PurchaseOrder().Master().getConvertedTransactionStatus().equals(PurchaseOrderStatus.CONFIRMED)) {
            ...
            CustomCommonUtil.setDisable(!lbShow,
                    ...
                    chkbAdvancePayment);
            CustomCommonUtil.setDisable(true, tfDiscountRate, tfDiscountAmount, tfBarcode, tfDescription,
                    tfAdvancePRate, tfAdvancePAmount, tfBrand, tfModel, tfCost, tfOrderQuantity);

            CustomCommonUtil.setDisable(!lbShow, tfCost, tfOrderQuantity);
            ...
            if (poPurchasingController.PurchaseOrder().Master().getTranTotal().doubleValue() > 0.000) {
